...
├── Agent_ASR_V3(history)      # 历史版本(使用关键词+轻量化的BERT路由，降低Websearch的深度，减少上下文token数)
...
| **LLM端到端总延迟**              | **7s - 9s**         | 9s - 12s          | 14s - 25s           |
| **关键词+BERT端到端总延迟(路由50ms)** | **5s - 6s**         | 7s - 10s          | 12s - 23s           |
...
* **LLM (想)**: `6.0s - 8.0s` (DeepSeek-V3 API + 联网搜索)--改进：意图识别本地化-BERT+关键词，减少一次API调用，搜索结果截断，降低API处理量
