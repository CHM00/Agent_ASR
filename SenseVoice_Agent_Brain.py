...
class SmartAgentBrain:
    ...
    async def stream_chat_llm(
        ...
        temperature=0.7,
        trace=None,
        generation_name: str = "llm.chat",
        generation_meta: Dict[str, str] | None = None,
    ) -> AsyncGenerator[str, None]:
        """封装 LLM 流式调用接口"""
        generation_obs = self.monitor.start_generation(
            trace,
            name=generation_name,
            model=self.LLM_MODEL,
            input_payload=messages,
            metadata={"temperature": temperature, **(generation_meta or {})},
        )
        collected = ""
        ...
        if self.LOCAL_LLM:
            ...
            collected += full_reply
            yield full_reply
            self.monitor.end_observation(generation_obs, output=collected)
        else:
            try:
    ...
    async def process_user_query(self, user_text, user_id, trace=None) -> AsyncGenerator[str, None]:
        """核心意图处理函数：带有上下文动态压缩"""
        print(f"处理请求 [流式]，用户: {user_id}")
        start_ts = time.perf_counter()
        owns_trace = trace is None
        if trace is None: # 一般来说trace应该由外部传入，但如果没有，就自己创建一个新的trace
            trace = self.monitor.start_trace(
                name="agent_turn",
                user_id=str(user_id),
                session_id=str(user_id),
                input_payload={"user_text": user_text},
                metadata={"component": "SmartAgentBrain", "local_llm": self.LOCAL_LLM},
            )
        ...
        # 2. 意图路由
        intent_span = self.monitor.start_span(trace, name="intent_route", input_payload=user_text)
        intent = await self._route_intent(user_text)
        self.monitor.end_observation(intent_span, output=intent)
        print(f"意图分析: {intent}")

        clarification_prompt = str(intent.get("Clarification_Prompt") or "").strip()
        if intent.get("Need_Clarification") and clarification_prompt:
            yield clarification_prompt
            self._post_process(user_text, clarification_prompt, user_id)
            if owns_trace:
                self.monitor.end_observation(
                    trace,
                    output={"reply": clarification_prompt, "branch": "clarification"},
                    metadata={"latency_ms": round((time.perf_counter() - start_ts) * 1000, 2)},
                )
                self.monitor.flush()
            return

        # 3. 技能编排
        skill_span = self.monitor.start_span(trace, name="skill_dispatch", input_payload=intent)
        skill_result = await self.skill_orchestrator.dispatch(intent)
        self.monitor.end_observation(
            skill_span,
            output={
                "handled": skill_result.handled,
        ...
        if skill_result.handled and skill_result.action == "order_food":
            reply = skill_result.reply or "抱歉，我暂时无法处理点餐请求。"
            yield reply
            full_response = reply

        # --- 技能分支 B: 联网搜索（检索由 Skill 执行） ---
        elif skill_result.handled and skill_result.action == "web_search":
            if skill_result.reply and not skill_result.search_context:
                ...
                async for chunk in self.stream_chat_llm(
                    msgs,
                    temperature=0.1,
                    trace=trace,
                    generation_name="llm.web_search_answer",
                    generation_meta={"branch": "web_search"},
                ):
        ...
        # --- 技能分支 C: 注册声纹 ---
        elif skill_result.handled and skill_result.action == "register_voice":
            yield skill_result.reply
            return

        # --- 技能分支 D: 其他已处理技能（车载控制等） ---
        elif skill_result.handled:
        ...
        # --- 分支 E: 闲聊/通用对话（LLM兜底） ---
            ...
            async for chunk in self.stream_chat_llm(
                msgs,
                trace=trace,
                generation_name="llm.general_chat",
                generation_meta={"branch": "chat"},
            ):
                sentence_buffer += chunk
                if any(p in chunk for p in split_punc):
                    yield sentence_buffer.strip()
        ...
        if owns_trace:
            self.monitor.end_observation(
                ...
                output={
                    "reply": full_response,
                    "skill_handled": skill_result.handled,
    ...
    async def _route_intent(self, user_text: str) -> dict:
        """优先使用 LLM 技能路由，失败后回退到规则 / BERT。"""
        return await self.intent_router_service.route_async(user_text)
